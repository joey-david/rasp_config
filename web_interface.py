...
*{box-sizing:border-box}body{margin:0;background:#050505;color:#f7f7fb;font-family:system-ui,sans-serif;overflow:hidden}.stream{position:fixed;inset:0;width:100vw;height:100vh;object-fit:cover;background:#000}.shade{position:fixed;inset:0;background:linear-gradient(180deg,rgba(0,0,0,.2),transparent 40%,rgba(0,0,0,.65));pointer-events:none}.bar{position:fixed;left:12px;right:12px;bottom:12px;display:flex;gap:10px;align-items:center;flex-wrap:wrap;padding:12px;border:1px solid #ffffff24;border-radius:22px;background:#080a0dcc;backdrop-filter:blur(18px);box-shadow:0 18px 60px #0009}.brand{font-weight:850;min-width:120px}.muted{font-size:12px;color:#b8bac4}.group{display:flex;gap:8px;align-items:center;padding:8px 10px;border-radius:16px;background:#ffffff14}.stat{min-width:58px}.label{display:block;font-size:10px;text-transform:uppercase;letter-spacing:.08em;color:#b8bac4}.value{font-weight:800;font-size:13px}button{border:0;border-radius:14px;height:38px;padding:0 14px;font-weight:850}.danger{background:#fb7185;color:#1b0205}.key{display:inline-grid;place-items:center;width:28px;height:28px;border-radius:9px;background:#ffffff18;color:#b8bac4;font-size:12px;font-weight:850}.on{background:#f7f7fb;color:#080a0d}input{accent-color:white}.err{flex-basis:100%;font:12px ui-monospace,monospace;color:#fecdd3;white-space:pre-wrap}.hide{display:none}@media(max-width:760px){.bar{max-height:45vh;overflow:auto}}
</style><img class="stream" src="/stream.mjpg"><div class="shade"></div><main class="bar" tabindex="0">
...
const down=new Set(), valid=["w","a","s","d"], $=id=>document.getElementById(id), power=$("power");
...
function applyState(s){if(!s.motion)return;$("left").textContent=s.motion.left+"%";$("right").textContent=s.motion.right+"%";$("status").textContent=s.motion.direction+" · "+s.motion.speed+"%";const c=s.camera.settings;$("fps").value=c.fps;$("crop").value=c.crop;$("hflip").checked=!!c.hflip;$("vflip").checked=!!c.vflip;$("fpsOut").textContent=c.fps;$("cropOut").textContent=c.crop+"%";$("err").textContent=s.camera.error||"";$("err").classList.toggle("hide",!s.camera.error)}
...
window.addEventListener("blur",stopNow);power.addEventListener("input",drive);for(const id of ["fps","crop","hflip","vflip"])$(id).addEventListener("input",settings);setInterval(()=>{if(down.size)drive()},100);fetch("/api/state").then(r=>r.json()).then(applyState);document.querySelector("main").focus();paintKeys();
</script>'''
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if p == "/drive":
            return self.send(200, robot.drive_keys(b.get("keys", ""), b.get("power")))
        if p == "/motion/stop":
            return self.send(200, robot.stop())
        if p == "/camera/settings":
            robot.camera.apply_settings(**b); return self.send(200, robot.status())
        self.send_error(404)
    ...
    def do_GET(self):
        ...
        if p == "/api/state":
            return self.send(200, robot.status())
        if p == "/snapshot.jpg":
